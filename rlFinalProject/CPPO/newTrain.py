...
def train():
    ...
    CPPO = PPO(state_dim, action_dim, lr_actor, lr_critic, gamma, K_epochs, eps_clip, has_continuous_action_space,
               action_std)
    CPPO = CPPO.to(device)
    ...
    while time_step <= max_training_timesteps:
        ...
        for t in range(1, max_ep_len + 1):
            ...
            state1 = torch.squeeze(state[0]).to(device)
            state2 = torch.squeeze(state[1]).to(device)
            state = torch.cat((state1, state2), dim=0).to(device)
            action = CPPO.select_action(state)
            ...
            CPPO.buffer.rewards.append(rews1)
            CPPO.buffer.is_terminals.append(bool(done[0]))
            time_step += 1
            if flag:
            ...
            if has_continuous_action_space and time_step % action_std_decay_freq == 0:
                CPPO.decay_action_std(action_std_decay_rate, min_action_std)
...
if __name__ == '__main__':
    ...
    np.save('CPPO_wheel_1.npy', finalSave)
